...
const Choice = function (name) { this.name = name;};
const jsMap = {
...
async function generateFiles(data, path) {
  const files = data.split(' ').map(file => `${file}.html`);
  files.forEach(file => {
    fs.closeSync(fs.openSync(`${succeeded.project_path}${path}/${file}`, 'w'));
    console.log(`generated ${file} generated`);
  });
};
...
const includeScripts = function (data) {
  data.forEach(script => {
    ...
    if (scriptName !== undefined) {
      copyFile(`../source/js/${scriptName}`, `/script/${scriptName}`);
    };
  });
};
...
const includeCss = function (data) {
  data.forEach(cssFile => {
    ...
    if (cssName !== undefined) {
      copyFile(`../source/css/${cssName}`, `/css/${cssName}`);
    };
  });
};
